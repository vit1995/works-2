...
function uniqArray(array) {
  return array.filter((item2, index, self2) => self2.indexOf(item2) === index);
}
...
function spollers() {
  ...
  if (spollersArray.length > 0) {
    ...
    const spollersRegular = Array.from(spollersArray).filter(function(item2, index, self2) {
...
document.addEventListener("DOMContentLoaded", function() {
  ...
  menuItems.forEach((item2, index) => {
    const link = item2.querySelector(".menu__link");
    const subMenu = item2.querySelector(".sub-menu");
...
class DynamicAdapt {
  ...
  init() {
    ...
    this.mediaQueries = this.objects.map(({ breakpoint }) => `(${this.type}-width: ${breakpoint / 16}em),${breakpoint}`).filter((item2, index, self2) => self2.indexOf(item2) === index);
...
function masonryRun() {
  ...
  const isotopeItems = new Isotope(isotope2, {
    ...
    layoutMode: "masonry",
    masonry: {
      columnWidth: "[data-fls-masonry-item]",
      gutter: 20,
      fitWidth: false
      // УБИРАЕМ fitWidth
    },
  ...
  function documentActions(e) {
    const targetElement = e.target;
    const filterLink = targetElement.closest("[data-fls-masonry-filter-link]");
    if (filterLink) {
      const filterValue = filterLink.dataset.flsMasonryFilterLink;
  ...
  imagesLoaded(isotope2, function() {
    isotopeItems.layout();
  });
}
if (document.querySelector("[data-fls-masonry]")) {
...
function showMore() {
  ...
  if (showMoreBlocks.length) {
    showMoreBlocksRegular = Array.from(showMoreBlocks).filter(function(item2, index, self2) {
